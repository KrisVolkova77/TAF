...
public class SerializationTest {
    ...
    @Test
    public void objectToJasonTest() throws IOException { //серилизация

        Gson gson = new Gson();
        Project project = new Project("Test Project","Big test data...",false, 2);

        String objectJason = gson.toJson(project);
        System.out.println(objectJason);

        FileWriter file = new FileWriter("projectResult.json");
        gson.toJson(project,file);
        file.close(); //обязательно незабываем закрывать файл

    }

    @Test
    public void jsonToObjectTest() throws IOException { //десерилизация
        Gson gson = new Gson();
