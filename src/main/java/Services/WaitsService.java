...
public class WaitsService {
    ...
    public WebElement fluentWaitForElement(By locator) {
        Wait<WebDriver> fluent = new FluentWait<>(driver)
                .withTimeout(Duration.ofSeconds(30)) //сколько всего ждать
                .pollingEvery(Duration.ofSeconds(50)) // проверять через каждые
                .ignoring(NoSuchElementException.class); //игнорируем Exception и проверяем дальше
